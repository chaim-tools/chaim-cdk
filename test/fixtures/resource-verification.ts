...
export async function verifySchemaDataMatchesFile(
  ...
  if (outputSchema.entity.primaryKey.partitionKey !== expectedSchema.entity.primaryKey.partitionKey) {
    throw new Error(
      `Partition key mismatch: expected ${expectedSchema.entity.primaryKey.partitionKey}, got ${outputSchema.entity.primaryKey.partitionKey}`
    );
  }
  
  // Verify field count matches
  if (outputSchema.entity.fields.length !== expectedSchema.entity.fields.length) {
    throw new Error(
      `Field count mismatch: expected ${expectedSchema.entity.fields.length}, got ${outputSchema.entity.fields.length}`
    );
  }
}
